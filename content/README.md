...
`accepted` съдържа всички приети отговори. Сравнението пренебрегва регистъра, излишните интервали и пунктуацията. `normalize` променя това: `caseSensitive`, `keepPunctuation` (за задачи със запетаи) и `pattern` (регулярен израз, който също приема отговора). `pattern` се сравнява с целия отговор след нормализирането, затова `^` и `$` не са нужни.
...
Поправеният текст се сравнява с оригинала: грешката носи точка, ако на мястото ѝ стои една от приетите поправки. Затова фрагментът трябва да обхваща мястото на поправката — при липсваща запетая думите от двете ѝ страни (`разказа че` → `разказа, че`). Правите и българските кавички се приемат еднакво, тирето и малкото тире — не. Промени извън грешките се отчитат като излишни.
...
Оценяване: пълен приет отговор — 2 точки; иначе 1 точка, ако поне една част е вярна. Отговор над 6 думи или без ключовата дума — 0 точки. Ако е написано цялото второ изречение, се взима само частта на мястото на `[[1]]`.
